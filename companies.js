    ...
    <div class="detail-grid">
      ...
      <div>
        ...
        ${(accountsRes.data || []).map(a => `
          <div class="list-item">
            <span class="stamp small">${a.account_number}</span> ${escapeHtml(a.name_no)}
            ${a.is_default ? '<span class="badge badge-class" style="margin-left:6px;">default</span>' : ''}
          </div>
        `).join('') || `<div class="muted small">No accounts linked yet.</div>`}
      </div>
    </div>
  `;
}

function showAddCompanyModal(root) {
  ...
  overlay.innerHTML = `
    <div class="modal">
      ...
      <form id="add-company-form">
        ...
        <input type="text" id="c-org" />
        <label>Industry</label>
        <input type="text" id="c-industry" />
        <label>Invoice contact email</label>
        <input type="email" id="c-email" placeholder="invoices@company.no" />
        <div style="display:flex; gap:10px; margin-top:20px;">
          <button type="button" class="btn-ghost" id="cancel-add">Cancel</button>
          <button type="submit" class="btn-primary" style="margin-top:0;">Save company</button>
        </div>
        <p id="add-company-error" class="error-text" hidden></p>
      </form>
    </div>
  `;
  ...
  overlay.querySelector('#add-company-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const name = overlay.querySelector('#c-name').value.trim();
    const org = overlay.querySelector('#c-org').value.trim();
    const industry = overlay.querySelector('#c-industry').value.trim();
    const email = overlay.querySelector('#c-email').value.trim();

    const { data: company, error } = await supabase.from('companies')
      .insert({ name, org_number: org || null, industry: industry || null, created_by: getProfile()?.id })
      .select().single();

    if (error) {
      const errEl = overlay.querySelector('#add-company-error');
      errEl.textContent = error.message;
      errEl.hidden = false;
      return;
    }
    if (email) {
      await supabase.from('company_contacts').insert({ company_id: company.id, contact_type: 'invoice', email });
    }
    overlay.remove();
    loadList(root);
  });
